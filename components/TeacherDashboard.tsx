...
import { TeacherInsight, Student, User } from '../types';
...
const TeacherDashboard: React.FC<TeacherDashboardProps> = ({ user, isAuthenticated, setIsAuthenticated, students, setStudents }) => {
  ...
  // Deriving Chart Data from actual student list
  const chartData = students.length > 0 ? [
    { name: 'A Grade', count: students.filter(s => s.grade.startsWith('A')).length },
    { name: 'B Grade', count: students.filter(s => s.grade.startsWith('B')).length },
    { name: 'C/D Grade', count: students.filter(s => s.grade.includes('C') || s.grade.includes('D')).length },
    { name: 'At Risk', count: students.filter(s => s.status === 'At Risk').length },
  ] : [
    { name: 'A Grade', count: 5 },
    { name: 'B Grade', count: 3 },
    { name: 'C/D Grade', count: 2 },
    { name: 'At Risk', count: 1 },
  ];
  ...
  useEffect(() => {
    if (isAuthenticated) {
      const fetchInsights = async () => {
        setLoading(true);
        // Feed real student summaries to the AI for insights
        const studentSummary = students.map(s => `${s.name}: ${s.grade} (${s.status})`).join(', ');
        const res = await generateTeacherInsights(studentSummary || "No students registered yet.");
        setInsights(res || []);
        setLoading(false);
      };
      fetchInsights();
    }
  }, [isAuthenticated, students.length]); // Re-fetch if auth or count changes
  ...
  const handleExport = () => {
    setIsExporting(true);
    setTimeout(() => {
      // Create CSV Content from current insights
      const headers = "Topic,Average Score,AI Recommendation\n";
      const rows = insights.map(d => `"${d.topic}",${d.avgScore}%,"${d.recommendation}"`).join("\n");
              ...
              {(insights || []).map((insight, idx) => (
                <div key={idx} className="p-5 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border dark:border-slate-700 hover:border-indigo-400 dark:hover:border-indigo-500 transition-all group">
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <span className={`text-[10px] px-2 py-1 rounded-lg font-black tracking-tight ${insight.avgScore < 50 ? 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400' : 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400'
                      }`}>
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {filteredStudents.map((student) => (
                        <tr key={student.id} className="hover:bg-slate-50 transition-colors">
                          ...
                          <td className="px-6 py-4">
                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${student.grade.startsWith('A') ? 'bg-green-100 text-green-800' :
                          ...
                          <td className="px-6 py-4">
                            <span className={`text-xs font-bold ${student.status === 'At Risk' ? 'text-red-500' :
